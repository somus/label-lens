...
- `sidebar: "auto" | "on" | "off"` — right-column sidebar (wordmark + nav + counters + signals + history, or stats totals) in place of the top status bar. Auto shows at ≥120 cols with truecolor / 256-color terminals; `on` forces visible regardless of width / capability; `off` keeps the top status bar and falls the review-screen history block back to a horizontal strip below the decision pane. Width scales as `max(32, min(64, floor(terminalWidth * 0.30)))` (`sidebarWidth`).
- `queuePreview: "auto" | "on" | "off"` — left-column rail listing upcoming records in the current queue (28ch fixed). Auto shows at ≥200 cols when the sidebar is also visible; `on` forces visible regardless of width; `off` hides. Read-only ambient surface — `j` / `k` scroll the cursor as usual and the rail follows.
...
**Per-label key accelerators** (PRD §10.1). `config.labels[].key` binds a single char to a relabel decision (e.g. `{ name: "food", key: "f" }`). `runReview` builds a dynamic registry of `relabelByKeyCommand` entries on top of `ALL_COMMANDS` and passes it into `mountReviewScreen({ registry })`. Keys flow through the keymap engine like any other binding — the chip rail and the relabel picker share `labelChipText` (`src/render/label-chip.ts`) so `[k]` / `[N/k]` rendering stays consistent. `validateLabelKeys` runs right after config parse and rejects collisions against `reservedReviewKeys(ALL_COMMANDS)` (single-char + chord-starter bindings, review + global scope, includes digits `1-9`) and duplicates across labels — aborting with `process.exit(2)`. `display.labelChip` (default `"configured"`) toggles the chip format. Labels past chip-rail position 9 still resolve through the dynamic command but only render via the picker; the `+N more (r)` hint surfaces `(some bind shortcuts)` when any hidden label has a configured key.

**LLM assistant** (PRD §10.5 + §14.4 + ADR 0009 — the inline-footer decision supersedes PRD §14.4's original right-side `ScrollbackSurface` panel). `i` opens the inline footer overlay (bottom strip, not a side panel). First press over `assistant.enabled: false` routes through the `configure-assistant` step machine (`src/overlay/configure-assistant.ts`): provider → API key or Ollama URL → privacy notice → commit. Persistence lives in the `updateAssistantConfig` effect; in-memory `app.config.assistant` is mutated immediately so the next `i` press fires the query. Provider call goes through `queryAssistant` (`src/assistant/provider.ts`): cache lookup by `(record_id, prompt_hash)` first, then `pi-ai` `stream()` with a single required `submit_label_suggestion` tool whose TypeBox parameters are `AssistantResponseSchema`. Streaming `text_delta` events route into the overlay reducer via `dispatchOverlayEvent` (`src/overlay/effects.ts`); `streamEnd` carries the parsed `AssistantResponse`. `Tab` toggles inline reasoning expand; `Enter` commits the recommended action with `source_of_truth: "human+assistant"`; `Esc` dismisses (still tags as viewed per ADR 0004). `record.next/prev` clear `viewedAssistant` so each focus session is fresh. `--local-only` blocks remote providers at `runReview` startup AND at the provider boundary.
