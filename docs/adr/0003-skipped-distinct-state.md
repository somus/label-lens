...
Review states are mutually exclusive: `pending`, `accepted`, `relabeled`, `rejected`, `skipped`. **`skipped` is its own state** — it does not appear in the `pending` queue, and it is not counted as `pending` in progress stats. Skipped records live in a dedicated `skipped` queue and are surfaced separately on the stats surface.
