...
**Responsive layout (future-proofing).** The current §14.1 layout stacks context-before, candidate, and context-after vertically. On wide terminals (≥160 columns) a side-by-side layout — context on the left, candidate centered, history strip and metadata on the right — is more efficient. MVP ships stack-only, but the render-primitives wrapper (§16.1) is structured so that a future layout switch is a single component-level decision keyed on terminal width, not a screen rewrite. (Hunk uses this pattern for split-vs-stack diff layout; same idea, different content.)
