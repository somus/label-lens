...
> **Version 2.9.** Update over v2.8: §18 V1 section restructured into per-feature subsections (V1.1 – V1.11) covering active learning / prioritization (phased), bulk operations (mark-and-batch), embedding / similarity, confidence calibration, rationale capture, inline highlights, multi-line entries, block / entry coloring, versioned diff after re-ingest, per-correction rationale, and confidence threshold tuning. §10.1 picks up the per-label config key binding (`config.labels[].key`) as an MVP addition. No behavior change to the review loop yet — V1 items remain deferred until the MVP loop is polished. Earlier change log preserved.
>
...
- Accept prediction
- Reject prediction
- Relabel record (single-key 1–9 for the first 9 labels; fuzzy picker for any label). Labels may override their accelerator key via `config.labels[].key` — when set, the configured key fires the relabel directly (e.g. `f` → `food`), bypassing the numbered chip. Honored across the chip rail and picker overlay.
- Skip record
...
Larger features deferred until the MVP loop is polished. Each item below was grilled in the v2.9 design pass — scope, design choices, and open questions are captured.
...
**Scope.** After a predictions-only re-ingest (ADR 0002), show the reviewer which predictions changed since the prior version.

**Design.**
- Diff covers prediction changes only. Text changes go through the existing re-ingest text-change flow; reviews never change on re-ingest (ADR 0007).
...
- Multi-label classification (with toggle UI and `Space` / `Enter` semantics). Slots into the `TaskRenderer` abstraction added in v2.8.
