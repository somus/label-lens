...
export function applyEffects(
  ...
  // Snapshot + reset so a hydrate that happens inside this call is
  // observable to the `close` effect (see context.ts/hydrateAssistant­
  // FromCache for the flag's purpose). Restored after the loop so
  // nested calls — e.g. a `runCommand` effect that re-enters
  // applyEffects via dispatchCommand — don't observe a stale flag.
  const priorHydrated = app.overlayHydratedThisBatch;
  app.overlayHydratedThisBatch = false;
  try {
    for (const effect of effects) {
      applyOneEffect(app, queueId, effect, dispatchCommand);
    }
  } finally {
    app.overlayHydratedThisBatch = priorHydrated;
  }
}
...
function applyOneEffect(
  ...
  switch (effect.kind) {
    case "close":
      // Earlier effects in the same batch — notably `commitDecision`'s
      // queue refresh, which triggers `cursor.on("change")` →
      // `hydrateAssistantFromCache` — may have installed a fresh
      // overlay for the newly-focused record. Preserve that instead of
      // calling `closeOverlay()`, which would clobber it.
      if (app.overlayHydratedThisBatch) {
        app.cancelAssistantStream();
        if (app.savedAssistantState) app.clearSavedAssistant();
      } else {
        app.closeOverlay();
      }
      break;
